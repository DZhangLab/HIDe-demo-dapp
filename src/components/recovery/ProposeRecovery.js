    ...
    return (
      <div className="App">
        <header className="App-header">
          ...
          <h6>The user lost their private key and they contacted you off chain. You have verified that authenticy of this request.
              By starting the recovery process you will create a proposal to change the current user public address to a new address.
              This address should be supplied by the user. Once proposed, delegates will have one week to vote. If vote yes has 50% or more,
              the address will automatically changed. Otherwise, the proposal will expire after 1 week and the address will change if the yes 
              votes is greater than or equal to no votes.
          </h6>
          ...
          />
          <button className="btn btn-outline-secondary" onClick={proposeRecovery}>
              Propose
            </button>
        </header>
        <br></br>
        ...
        <CardContent>
          ...
          <Typography variant="h7" component="div">
           Status: {parseInt(proposal?.status?._hex, 16) == 0 ? "Cancelled" : parseInt(proposal?.status?._hex, 16) == 1? "Pending" :"Cancelled"}
          </Typography>
