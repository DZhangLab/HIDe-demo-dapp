...
async function main() {
  ...
  const hashData1 = await createAndPublishDIDJson(
    "did:hide:abc123",
    (
      await dapp
    ).address,
  ...
  const hashData2 = await createAndPublishDIDJson(
    "did:hide:def456",
    (
      await dapp
    ).address,
  ...
  console.log("DApp Contract deployed to: ", (await dapp).address);
  console.log(
    "Controller1 Contract deployed to: ",
    (await controller1).address
  );
  console.log(
    "Controller2 Contract deployed to: ",
    (await controller2).address
  );
}
...
async function createAndPublishDIDJson(
  DID,
  controller,
  blockchainAccountID,
  timestamp
